...
module.exports = function crex24 (conf) {
  ...
  function publicClient () {
    if (!public_client) public_client = new ccxt.crex24({ 'apiKey': '', 'secret': '' })
    return public_client
  }

  function authedClient() {
    if (!authed_client) {
      if (!conf.crex24 || !conf.crex24.key || !conf.crex24.key === 'YOUR-API-KEY') {
        throw new Error('please configure your CREX24 credentials in ' + path.resolve(__dirname, 'conf.js'))
      }
      authed_client = new ccxt.crex24({ 'apiKey': conf.crex24.key, 'secret': conf.crex24.secret })
    }
  ...
  function retry (method, args, err) {
    var timeout = 5000
    if (method == 'getOrder') {
      // it can take up to 30 seconds for crex24 to update with an order change.
      if (err)    
